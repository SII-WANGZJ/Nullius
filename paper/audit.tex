...
\author{[Author]\\[2pt]\small [Affiliation]}
\date{\today}
...
% TODO: expand each into 2--3 sentences.
\begin{enumerate}\itemsep2pt
  \item Disclose backbone model, version and training cut-off relative to all
        target papers.
  \item Report a prior-knowledge control: $N$ independent single-shot queries
        to the bare backbone from the same platform description, scored for
        whether the mechanism is proposed and whether it is correctly
        identified as standard coherent detection.
  \item Perform the operator-matched equality test of Sec.~\ref{sec:operator}.
  \item Re-evaluate on a benchmark with genuine relational structure, with
        pair-grouped splits and self-pairs excluded, against the
        measurement-free identity-rule baseline.
  \item Release the agent trace, intervention log and $\geq 3$ independent
        runs.
\end{enumerate}
