...
Yang et al.~\cite{qiushi}, the audited work throughout, couple an LLM multi-agent system to a
...
Three further terms recur below. A \emph{codebook} is the fixed mapping from
token index to the random-phase input field that encodes it. A \emph{full
...
is the number of channels retained. \emph{Pseudo-$B$} denotes the authors'
control in which the physical transmission operator is replaced by a random
...
\bibitem{lightening} H.~Zhu \emph{et al.},
  ...
  High-Performance Computer Architecture (HPCA), 686--703 (2024).
