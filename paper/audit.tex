...
removing them from either side returns accuracy to chance. The second
experiment supporting the same conclusion, a four-token XOR showcase, behaves
the same way: its released accuracy of $1.000$ is obtained under a
sample-level split and is not retained under any split that keeps a pair's
repeated measurements together. We emphasise what we do \emph{not} claim: we find no evidence in the inspected data that the
...
excluded from training entirely. None of these is distinguishable from chance
at this sample size, and their non-monotone ordering should not be
interpreted.
