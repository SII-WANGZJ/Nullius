...
removing them from either side leaves no evidence of above-chance transfer.
The second experiment supporting the same conclusion, a four-token XOR
showcase, behaves the same way: its released accuracy of $1.000$ is obtained
under a sample-level split and is not retained under any split that keeps a
pair's repeated measurements together. What both benchmarks establish is
decodability of the fixed, already-measured pairs, not generalisation of a
relation to unseen pairs or unseen tokens. We emphasise what we do \emph{not}
claim: we find no evidence in the inspected data that the
...
that carries the computational conclusion. Neither benchmark is contradicted
as a demonstration that repeated measurements of a fixed, already-observed
pair library are decodable; what they do not support is the relation-specific,
semantic, and unseen-pair reading placed on them.
\end{abstract}
...
\toprule
Claim & Public material shows & Audit test & Outcome & Minimum conclusion that stands \\
\midrule
XOR interaction & sample-level $1.000$ & pair, mirror and token grouping
& not retained & repeated measurements of the 16 fixed pairs are decodable \\
\addlinespace
Semantic relation & high sample-level accuracy & grouped CV plus exact label
permutation & designated partition not exceptional ($102/105$) & pair identity
is decodable \\
\addlinespace
Task D transfer & Complex-B above chance & identity rule and $2\times2$
ablation & learned self-pair shortcut & a decodable difference exists under
the fixed protocol \\
\addlinespace
Attention analogy & a bilinear observable & operator comparison
& weak form only & a fixed physical pair-feature primitive \\
\addlinespace
Physical novelty & a measurable cross term & internal consistency of the
released characterisation & not established & the cross term is measurable on
the platform \\
\addlinespace
Autonomous discovery & trajectory narrative and resource counts &
provenance and ablation audit & not attributable & the automation may be
substantial; this audit does not evaluate it \\
\bottomrule
\end{tabular}
\end{table}
