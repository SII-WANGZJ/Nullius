...
% A long path in \code cannot break and overruns the measure. \filepath{} is
% set verbatim by url.sty, which is told below to break at / and _.
\usepackage{url}
\urlstyle{tt}
\def\UrlBreaks{\do\/\do\_\do\-\do\.}
\newcommand{\filepath}{\begingroup\urlstyle{tt}\Url}
...
\vspace{0.7em}
\noindent\textbf{Keywords:} reproducibility; data leakage; cross-validation;
benchmark evaluation; autonomous scientific discovery; optical computing.
\end{abstract}
...
available to us as of 30 July 2026: version v1 of the preprint~\cite{qiushi}
and the Zenodo deposit~\cite{zenodo}. The deposit states that the formal
Supplementary
...
free-space optical platform and report three studies of increasing autonomy,
culminating in an open-ended investigation that identifies an ``optical
...
All material below derives from the authors' own deposit~\cite{zenodo}.
Our pipeline mirrors the released analysis script
\filepath{05_result4_refine_paper/scripts/analyze_advantage_boundary.py},
